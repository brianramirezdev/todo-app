...
function PaginationPrevious({ className, title, ...props }: React.ComponentProps<typeof PaginationLink>) {
    return (
        <PaginationLink aria-label="Go to previous page" size="default" className={cn('gap-1 px-2.5 sm:pl-2.5', className)} {...props}>
            <ChevronLeftIcon />
            <span className="hidden sm:block capitalize">{title ?? 'Previous'}</span>
        </PaginationLink>
    );
}

function PaginationNext({ className, title, ...props }: React.ComponentProps<typeof PaginationLink>) {
    return (
        <PaginationLink aria-label="Go to next page" size="default" className={cn('gap-1 px-2.5 sm:pr-2.5', className)} {...props}>
            <span className="hidden sm:block capitalize">{title ?? 'Next'}</span>
            <ChevronRightIcon />
        </PaginationLink>
    );
}
